...
from nika.services.evaluation import EvaluationService
...
from nika.services.rag_service import RagAnswer, RagService, format_sources
...
class MessageHandler:
    def __init__(
        ...
        self._evaluation = EvaluationService(config, rag)
    ...
    async def handle_help(self, message: Message) -> None:
        await message.answer(
            ...
            "/evaluate_dataset — оценка качества RAG (RAGAS)\n"
    ...
    async def handle_evaluate_dataset(self, message: Message) -> None:
        ...
        try:
            result = await asyncio.to_thread(
                self._evaluation.evaluate_dataset,
                dataset_name,
            )
            ...
            await status.edit_text(
                "Не удалось выполнить evaluation. Проверь логи."
            )
            return
    ...
    async def handle_text(self, message: Message) -> None:
        ...
        try:
            reply = await self._handle_message(text, history, message)
        except Exception:
    ...
    async def handle_voice(self, message: Message) -> None:
        ...
        try:
            ...
            reply = await self._handle_message(transcript, history, message)
        except Exception:
    ...
    async def _handle_message(
        ...
        if extraction.is_reference_question:
            ...
            try:
                result = await self._rag.aanswer(text, history)
                return HandlerReply(
                    text=self._format_rag_response(result),
    ...
    def _format_rag_response(self, result: RagAnswer) -> str:
        response = result.answer
        if self._config.show_sources and result.documents:
