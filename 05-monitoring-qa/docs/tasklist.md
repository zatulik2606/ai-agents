...
| 32 | ДЗ-5: Документация и полировка | ⬜ Ожидает | README, .env.example, обработка ошибок |

**Легенда:** ⬜ Ожидает · 🔄 В работе · ✅ Готово · ⛔ Заблокировано

**Текущая итерация:** 32 — ДЗ-5: Документация и полировка  
**Завершено:** 31 / 32
...
- [ ] README: источники, LangSmith, dataset, evaluation, RAGAS-метрики
- [ ] Обработка ошибок (нет API-ключа, нет датасета, rate limits)
- [ ] Обновить `/help`
- [ ] Скопировать `data/` из референса (PDF + JSON Q&A)
