...
| Эмбеддинги | `MODEL_EMBEDDING` | Векторизация чанков PDF для RAG |

Модели могут совпадать (например, одна multimodal на всё), но в конфиге задаются отдельно.

### RAG (LangChain + OpenRouter)

Референс пайплайна — ноутбук `notebooks/naive-rag.ipynb`, финальная цепочка `rag_query_transform_chain`.

| Компонент | Решение |
|-----------|---------|
| Vector store | `InMemoryVectorStore` (in-memory, без персистентности) |
| Источник | PDF в `DATA_PDF` |
| Retriever | top-K, K = `RETRIEVER_K` из `.env` |
| Query transform | LLM переформулирует запрос с учётом истории до поиска |
...
│       ├── indexer.py         # PDF → chunks → InMemoryVectorStore
│       └── rag_service.py     # query transform + retrieve + answer
├── data/
│   ├── meals.json             # учёт (создаётся автоматически)
│   └── rukovodstvo_dlya_detei_i_ih_roditelei_saharnii_diabet_1_tipa_chto_neobhodimo_znat_178.pdf
├── notebooks/
│   └── naive-rag.ipynb        # референс rag_query_transform_chain
├── docs/
│   ├── idea.md
│   ├── vision.md
│   ├── tasklist.md
...
| Справочный вопрос | `RagService` — query transform + top-K + ответ с контекстом |
...
| `MODEL_EMBEDDING` | `openai/text-embedding-3-small` | Эмбеддинги для RAG |
| `RETRIEVER_K` | `4` | Top-K чанков из retriever |
...
PDF (DATA_PDF)
    ...
    → OpenAIEmbeddings (MODEL_EMBEDDING, base_url из Config)
    → InMemoryVectorStore
```
...
Цепочка по образцу `rag_query_transform_chain` из `notebooks/naive-rag.ipynb`:

```
вопрос + history (LangChain messages)
    → query transformation (MODEL_TEXT)
    → retriever.invoke(transformed_query)  # top RETRIEVER_K
