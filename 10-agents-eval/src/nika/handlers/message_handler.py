...
from nika.services.agent_service import AgentService, format_hitl_interrupt
...
class MessageHandler:
    def __init__(
        ...
        self._pending_interrupts: dict[int, object] = {}
        self.router = Router()
    ...
    def register(self) -> Router:
        ...
        self.router.callback_query.register(
            self.handle_hitl_callback,
            F.data.startswith("hitl_"),
        )
        return self.router
    ...
    async def handle_reset(self, message: Message) -> None:
        ...
        self._agent.reset_thread(user_id)
        self._pending_interrupts.pop(user_id, None)
    ...
    async def handle_help(self, message: Message) -> None:
        ...
        await message.answer(
            ...
            "order_care_product, register_cgm_sensor "
            "(MCP, требуют Accept/Reject)\n"
            "Безопасность: HITL, маскирование карт, лимиты вызовов\n"
            "MCP: сначала make run-mcp-nika, потом бот\n"
    ...
    async def handle_coeffs(self, message: Message) -> None:
        await message.answer(
            ...
            f"• Углеводный коэффициент: {self._config.carb_ratio:g} "
            f"(граммов углеводов на 1 ЕД — единицу инсулина)\n"
            f"• Чувствительность: {self._config.insulin_sensitivity:g} "
            f"(на сколько ммоль/л снижает сахар 1 ЕД)\n"
            f"• Целевой сахар мин: {self._config.target_glucose_min:g} ммоль/л\n"
            f"• Целевой сахар макс: {self._config.target_glucose_max:g} ммоль/л\n"
            f"• Коэффициент БЖЕ: {self._config.fpu_ratio:g} "
            f"(граммов белков+жиров на 1 ЕД; БЖЕ = белково-жировые единицы)"
        )
    ...
    async def handle_example(self, message: Message) -> None:
        await message.answer(
            ...
            "• Какие полоски к Contour? — search_products (MCP)\n"
            "• Сколько углеводов в банане? — food_nutrition (MCP)\n"
            "• Сколько ЕД на 48 г углеводов при К=12? — calculate_meal_bolus (MCP)\n"
            "• Закажи полоски Contour на имя IVAN PETROV — "
            "order_care_product + кнопки Accept/Reject\n"
            "• Зарегистрируй сенсор FreeStyle Libre 3 на имя IVAN PETROV — "
            "register_cgm_sensor + Accept/Reject\n"
            "• Найди что такое гипогликемия, потом полоски Contour, "
            "углеводы в банане и 180 мг/дл в ммоль — тест лимитов tools"
        )
    ...
    async def handle_text(self, message: Message) -> None:
        ...
        if reply.interrupt is not None:
            await self._send_hitl_request(message, reply.interrupt)
            return
    ...
    async def handle_hitl_callback(self, callback: CallbackQuery) -> None:
        ...
        hitl_message = callback.message

        try:
            action, chat_id_str = callback.data.split(":", maxsplit=1)
            chat_id = int(chat_id_str)
        except ValueError:
            await callback.answer("Некорректные данные кнопки", show_alert=True)
            return

        if chat_id not in self._pending_interrupts:
            await callback.answer("Запрос устарел", show_alert=True)
            return

        await hitl_message.edit_reply_markup(reply_markup=None)

        if action == "hitl_approve":
            decision = "approve"
            status_note = "\n\nОперация подтверждена (Accept)"
        else:
            decision = "reject"
            status_note = "\n\nОперация отклонена (Reject)"

        try:
            await hitl_message.edit_text(
                f"{hitl_message.text or ''}{status_note}"
            )
        except Exception:
            logger.exception("Failed to edit HITL message text")

        del self._pending_interrupts[chat_id]
        processing = await hitl_message.answer("Обрабатываю решение…")
        ...
        try:
            ...
            response = self._format_agent_response(result.answer, result.documents)
            await hitl_message.answer(response)
            self._history.add_assistant(chat_id, result.answer)
            logger.info("HITL %s processed for chat_id=%s", decision, chat_id)
        except Exception:
            logger.exception("HITL resume error for chat_id=%s", chat_id)
            await hitl_message.answer(
                "Не удалось обработать решение. Попробуй ещё раз."
            )
        finally:
    ...
    async def _send_hitl_request(
        self,
        message: Message,
        interrupt: object,
    ) -> None:
        chat_id = message.chat.id
        self._pending_interrupts[chat_id] = interrupt
        text = format_hitl_interrupt(interrupt)
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="Accept",
                        callback_data=f"hitl_approve:{chat_id}",
                    ),
                    InlineKeyboardButton(
                        text="Reject",
                        callback_data=f"hitl_reject:{chat_id}",
                    ),
                ]
            ]
        )
        await message.answer(text, reply_markup=keyboard)
        logger.info("HITL interrupt sent to chat_id=%s", chat_id)
