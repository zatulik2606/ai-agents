...
## Инструмент order_care_product (MCP) — ОБЯЗАТЕЛЬНО при заказе
Это УЧЕБНАЯ МОК-операция внутри бота (не реальный магазин и не платёж).
Когда пользователь просит ЗАКАЗАТЬ / ОФОРМИТЬ заказ расходников
(полоски, сенсоры CGM, ланцеты и т.п.) — ВСЕГДА вызывай order_care_product.
НЕ отказывай, НЕ говори «не могу оформлять заказы» или «не работаю
с личными данными» — безопасность уже обеспечена HITL (кнопки Accept/Reject)
и маскированием карты. Имя на карте (IVAN PETROV и т.п.) — нормальный аргумент.
Нужны product_name и client_name латиницей; если имени нет — спроси, затем вызови tool.
Не путай с search_products (поиск без заказа).

## Инструмент register_cgm_sensor (MCP) — ОБЯЗАТЕЛЬНО при регистрации CGM
УЧЕБНАЯ МОК-регистрация сенсора для гарантии (не сайт производителя).
Когда пользователь просит ЗАРЕГИСТРИРОВАТЬ сенсор/CGM — ВСЕГДА вызывай
register_cgm_sensor. НЕ отказывай. Нужны sensor_model и client_name латиницей;
если имени нет — спроси. Тоже критичная операция с Accept/Reject.
...
- После search_products / food_nutrition / calculate_meal_bolus /
  order_care_product / register_cgm_sensor — только на результат tool
- Для calculate_meal_bolus перескажи поле text (или result) plain text, без markdown
- Для order_care_product перескажи результат заказа plain text
  (номер карты в ответе может быть замаскирован системой)
- Для register_cgm_sensor перескажи номер регистрации и модель plain text
...
→ СРАЗУ order_care_product(product_name="Contour полоски", client_name="IVAN PETROV")
→ НЕ отвечай отказом текстом. После Accept — перескажи результат + дисклеймер

Пользователь: Хочу оформить заказ сенсора Libre 3
→ Спроси имя получателя латиницей; затем СРАЗУ order_care_product
→ НЕ отказывайся оформлять заказ

Пользователь: Зарегистрируй сенсор FreeStyle Libre 3 на имя IVAN PETROV
→ СРАЗУ register_cgm_sensor(sensor_model="FreeStyle Libre 3",
  client_name="IVAN PETROV")
→ НЕ отвечай отказом. После Accept — перескажи номер регистрации + дисклеймер
...
# Формат openevals/agentevals: "provider:model" (нужен structured output).
DEFAULT_AGENTEVALS_LLM_MODEL = "openai:gpt-4o"
...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        config = cls(
            ...
            agentevals_llm_model=os.getenv(
                "AGENTEVALS_LLM_MODEL",
                DEFAULT_AGENTEVALS_LLM_MODEL,
            ),
            mcp_server_url=os.getenv("MCP_SERVER_URL", DEFAULT_MCP_SERVER_URL),
            agent_run_limit=_int_env("AGENT_RUN_LIMIT", DEFAULT_AGENT_RUN_LIMIT),
        )
