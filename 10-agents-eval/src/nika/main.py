...
async def main() -> None:
    ...
    logger.info(
        "Agent config: mode=%s pdf=%s embedding_provider=%s embedding=%s model=%s "
        "semantic_k=%s bm25_k=%s hybrid_k=%s reranker_fetch_k=%s reranker_k=%s "
        "crossencoder=%s openai_base_url=%s mcp_url=%s mcp_tools=%s",
    ...
    logger.info(
        "Monitoring: show_sources=%s langsmith=%s project=%s dataset=%s",
        config.show_sources,
        langsmith_status,
