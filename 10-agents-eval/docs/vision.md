...
> (`search_products`, `food_nutrition`, `calculate_meal_bolus`, `order_care_product`
> с сервера `mcp-nika-agent`).
...
> E2E агента: `agentevals` (trajectory match + LLM-as-a-Judge) — `vision.md` §18.
> Безопасность: middleware HITL / PII / rate limits (`vision.md` §17).
...
| `agentevals` | Траекторная оценка агента (e2e-тесты) |
| `pytest` / `pytest-asyncio` | Запуск async e2e-тестов (dev) |
...
| MCP tools | `search_products`, `food_nutrition`, `calculate_meal_bolus`, `order_care_product` через MCP client (§16–17) |
| Middleware | HITL, PII mask, model/tool call limits (§17) |
...
| Инструмент | Назначение |
|------------|------------|
| `Makefile` | install, run, run-mcp-nika, test-*, docker-build, docker-run |
...
- ORM
- CI/CD (локальные e2e в `tests/` — да; пайплайн CI — нет)
...
│   ├── handlers/
│   │   └── message_handler.py # текст, фото, голос, команды, HITL callback
│   └── services/
│       ├── chat_history.py
│       ├── llm_client.py
...
│       ├── agent_service.py   # create_nika_agent + middleware + run_turn_agent
...
│       ├── data/
│       │   └── care_products.json
│       └── …                  # search_products / food_nutrition /
│                              # calculate_meal_bolus / order_care_product
├── tests/                     # e2e agentevals (§18)
│   ├── conftest.py
│   ├── helpers.py
...
├── pyproject.toml             # + langchain-mcp-adapters, agentevals, pytest*
├── Makefile                   # run, run-mcp-nika, test-*, …
└── Dockerfile
```
...
- **Промпт агента** (справочный диалог): отдельный `AGENT_SYSTEM_PROMPT` —
  ...
  `calculate_meal_bolus` / `order_care_product` / `glucose_unit_converter`,
  few-shot. Детали — §15–17.
...
| `AGENTEVALS_LLM_MODEL` | `openai:gpt-4o` | Модель LLM-as-a-Judge (agentevals); нужен structured output |
...
RAGAS оценивает **качество ответа/контекста**. Траектории tools — отдельно, §18.
...
- e2e / agentevals — `docs/references/agent-evaluation.ipynb` (см. §18)
- MCP-агент — `../slides/notebook/09-mcp/agent-mcp.ipynb` (раздел «Агент с MCP инструментами»)
- security / HITL — `../slides/notebooks/agent-guards-demo.ipynb`
  (раздел 5: «Версия 1 — Агент с human-in-the-loop», `run_turn_agent`)
...
**Получение ответа (`run_turn_agent`):**

Переписать вызов агента по референсу `run_turn_agent` из `agent-guards-demo.ipynb`
(раздел 5): один ход поддерживает обычный user text **и** resume после HITL.

- `agent.stream(..., stream_mode="values")` (или обёртка async при необходимости)
- Логирование каждого шага (отдельная функция)
- При interrupt — вернуть детали операции handler'у (кнопки Accept / Reject)
...
6. **`order_care_product`** — заказ расходников (мок); нужны продукт и имя клиента;
   критичная операция — пользователь подтвердит кнопками
7. **`glucose_unit_converter`** — ммоль/л ↔ мг/дл
8. Когда **не** вызывать tools (приветствие, chitchat)
9. Few-shot: вопрос → вызов нужного tool с удачными аргументами
10. Подсказки: follow-up → самостоятельный запрос; при необходимости повтор tool

### Граница KISS

- Локальные tools + MCP-tools (§16–17). Учёт/фото/инсулин остаются в handler +
...
- Security middleware — штатные классы LangChain; без своего «security framework».
...
### Tool `order_care_product` (MCP)

Аналог учебного `open_credit_card` из банковского демо: критичная операция в домене Ники.

| Аргумент | Смысл |
|----------|--------|
| `product_name` | название / тип расходника (полоски, сенсор CGM, …) |
| `client_name` | имя получателя латиницей (как на карте оплаты) |

- Мок-реализация: логирует заказ, возвращает номер заказа, продукт,
  **номер карты оплаты без маскирования** и срок действия (без CVV).
- Требует HITL (Accept / Reject) — см. §17.
- После Accept номер карты в ответе пользователю маскируется `PIIMiddleware`.

### Tool `register_cgm_sensor` (MCP)

Аналог учебного `open_deposit`: вторая критичная операция с HITL.

| Аргумент | Смысл |
|----------|--------|
| `sensor_model` | модель сенсора (FreeStyle Libre 3, Dexcom G6, …) |
| `client_name` | имя владельца латиницей |

- Мок: логирует регистрацию, возвращает номер регистрации и срок гарантии.
- Требует HITL (Accept / Reject) — см. §17.
...
## 17. Безопасность агента: HITL, PII, rate limits

Референс: `../slides/notebooks/agent-guards-demo.ipynb`
(разделы 5–7: HITL, PII, call limits; функция `run_turn_agent`).

Цель модуля — runtime-защита через **middleware** `create_agent`, без отдельного
security-фреймворка и без ручного StateGraph.

### Middleware stack (порядок слоёв)

| Слой | Middleware | Назначение |
|------|------------|------------|
| 1 | `ModelCallLimitMiddleware` | лимит вызовов LLM за run |
| 2 | `ToolCallLimitMiddleware` | защита от tool bombing |
| 3 | `PIIMiddleware("credit_card", …)` | маскирование номеров карт в **output** |
| 4 | `HumanInTheLoopMiddleware` | подтверждение `order_care_product` |

Конфигурация HITL (только accept / reject, **без edit**):
...
PII (в `agent_service.py`, не `src/agent.py`):
...
PIIMiddleware(
    ...
    apply_to_tool_results=True,  # маска и в результате MCP-tool
),
PIIMiddleware(
    "email",
    strategy="mask",
...
Источник открытого номера — мок `order_care_product`; в Telegram пользователь
должен видеть маску вида `****-****-****-5100`.
Email в ответах — маска (не полный `ivan.petrov@example.com`).

Лимиты — `AGENT_RUN_LIMIT` в `.env` (дефолт **3**), `exit_behavior="end"`.
Без пер-user Redis/квот.

Код в `agent_service.py` `_build_middleware()`:
...
PIIMiddleware("credit_card", strategy="mask", apply_to_input=False,
              apply_to_output=True, apply_to_tool_results=True),
HumanInTheLoopMiddleware(interrupt_on={...order_care_product, register_cgm_sensor...}),
```

Тест лимитов (домен Ники, не банк):

```
Найди что такое гипогликемия, потом какие полоски к Contour,
потом сколько углеводов в банане, и ещё переведи 180 мг/дл в ммоль/л
```

Агент должен gracefully завершиться при достижении лимита (не падать).

### Поток HITL в Telegram

```
пользователь: «Закажи полоски Contour на имя IVAN PETROV»
    → run_turn_agent(user_text=…)
    → агент вызывает order_care_product
    → HumanInTheLoopMiddleware → interrupt
    → бот показывает детали операции + кнопки Accept / Reject
    → callback → run_turn_agent(command=approve|reject)
    → кнопки удаляются (edit_reply_markup=None)
    → tool выполняется или отменяется → ответ пользователю
```

- Кнопки: подписи **Accept** / **Reject**; в LangChain decisions —
  `approve` / `reject`.
- После нажатия markup снимается, чтобы нельзя было нажать повторно.
- Pending interrupt хранить просто (in-memory dict по `chat_id`) — KISS.
- `MemorySaver` / `thread_id=chat_id` обязателен для resume после interrupt.

### Граница KISS (§17)

- Не реализуем edit параметров tool-call.
- Не добавляем InvisibleText / LLM Guard в обязательный scope (опционально позже).
- Не маскируем input; только output (номер карты из мок-tool).
- Не переносим HITL на учёт еды / фото / инсулин в handler — только MCP
  `order_care_product` / `register_cgm_sensor` (и при необходимости другие
  tools через тот же `interrupt_on`).

---

## 18. E2E-тесты агента (agentevals)

> Отдельный контур от RAGAS (§14). Здесь проверяем **траекторию**: какие tools
> вызвал агент и насколько это уместно. Референс —
> `docs/references/agent-evaluation.ipynb` (курс: `slides/notebook/agent-evaluation.ipynb`).
> Ориентир структуры — `live/tests/` курса.

### Цель

Автоматически проверять, что ReAct-агент Ники выбирает правильные tools
на типичных запросах (справочник, конвертация глюкозы, расходники).

### Стек

| Пакет | Назначение |
|-------|------------|
| `pytest` + `pytest-asyncio` | async e2e |
| `agentevals` | trajectory match + LLM-as-a-Judge |
| `MemorySaver` | изоляция сессий по уникальному `thread_id` |

### Типы тестов

#### 1. Trajectory Match (детерминированные)

Evaluator: `create_trajectory_match_evaluator` (или async-аналог из API).

Режимы match: `superset` / `subset` / `strict` / `unordered`.
Аргументы: обычно `tool_args_match_mode="ignore"` (LLM переформулирует query).

Быстрые, стабильные; не требуют отдельной judge-модели.

#### 2. LLM-as-a-Judge

Evaluator: `create_async_trajectory_llm_as_judge`.

Модель — `AGENTEVALS_LLM_MODEL` из `.env` (нужен structured output).
С референсной траекторией или без (`TRAJECTORY_ACCURACY_PROMPT*`).

Медленнее и дороже; гонять отдельно от match.

### Планируемые тесты (минимум 3)

| # | Файл | Тип | Сценарий | Evaluator |
|---|------|-----|----------|-----------|
| 1 | `test_agent_deterministic.py` | match | «Что такое гипогликемия?» → минимум `rag_search` | superset + ignore |
| 2 | `test_agent_deterministic.py` | match | «Переведи 180 мг/дл в ммоль/л» → `glucose_unit_converter` | superset + ignore |
| 3 | `test_agent_llm_judge.py` | LLM-judge | «Какие полоски к Contour Plus?» → уместная траектория с `search_products` | async judge + reference |

Тесты 1–2 — локальные tools, MCP не обязателен.
Тест 3 — нужен `make run-mcp-nika` (или fixture, поднимающий MCP).
...
tests/
├── __init__.py
├── conftest.py                 # agent fixture, уникальный thread_id
├── helpers.py                  # extract_trajectory, print_trajectory
├── test_agent_deterministic.py # match-evaluators
...
```bash
make test-deterministic   # pytest tests/test_agent_deterministic.py -v
make test-llm-judge       # pytest tests/test_agent_llm_judge.py -v
make test-all             # pytest tests/ -v
```

### Изоляция

- Уникальный `thread_id` / `chat_id` на тест (MemorySaver не смешивает сессии).
- Не мокать tools без необходимости — гоняем реальный агент (KISS).
- HITL-сценарии (Accept/Reject) — вне минимального набора из 3 тестов (YAGNI).

### Граница KISS (§18)

- Не дублировать RAGAS-метрики в pytest.
- Не строить CI/CD.
- Не тащить банковские сценарии из эталона курса — только домен Ники.
