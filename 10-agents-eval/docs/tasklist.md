...
### Спринт 9 — E2E оценка агента (agentevals)

| # | Итерация | Статус | Проверка |
|---|----------|--------|----------|
| 63 | ДЗ-10: Документация e2e | ✅ Готово | idea/vision §18 / conventions / tasklist |
| 64 | ДЗ-10: deps + config + Makefile | ✅ Готово | agentevals, pytest*; AGENTEVALS_LLM_MODEL; test-* |
| 65 | ДЗ-10: каркас tests/ | ✅ Готово | conftest, helpers, extract_trajectory |
| 66 | ДЗ-10: детерминированные тесты | ✅ Готово | rag_search + glucose_unit_converter (match) |
| 67 | ДЗ-10: LLM-as-a-Judge тест | ✅ Готово | search_products + async judge |
| 68 | ДЗ-10: README и полировка | ✅ Готово | README, .env.example, report.md |

**Легенда:** ⬜ Ожидает · 🔄 В работе · ✅ Готово · ⛔ Заблокировано

**Текущая итерация:** спринт 9 завершён  
**Завершено:** 68 / 68
...
> E2E траекторные тесты агента Ники через `agentevals`.
> RAGAS (`/evaluate_dataset`) не трогаем — отдельный контур (`vision.md` §14).
> Архитектура — `vision.md` §18.
> Референс — `docs/references/agent-evaluation.ipynb`.
> Учёт, фото, голос, hybrid/rerank, MCP, HITL сохраняются.
>
> Планируемые тесты (3):
> 1. match: гипогликемия → `rag_search` (superset)
> 2. match: 180 мг/дл → `glucose_unit_converter` (superset)
> 3. LLM-judge: полоски Contour → `search_products`
>
> Запуск:
> ```bash
> make run-mcp-nika        # для LLM-judge / MCP-сценариев
> make test-deterministic
> make test-llm-judge
> make test-all
> ```

### 63. ДЗ-10: Документация e2e

- [x] Обновить `docs/idea.md` — e2e agentevals, Makefile-таргеты
- [x] Обновить `docs/vision.md` — §18, структура `tests/`, `AGENTEVALS_LLM_MODEL`
- [x] Синхронизировать `.cursor/rules/conventions.mdc` со vision (без дублирования)
- [x] Добавить Спринт 9 в `docs/tasklist.md`
- [x] Положить `docs/references/agent-evaluation.ipynb`

**Проверка:** docs согласованы; conventions ссылаются на vision §18; запрет CI остаётся, e2e в scope.

---

### 64. ДЗ-10: deps + config + Makefile

- [x] `uv add` / dev: `agentevals`, `pytest`, `pytest-asyncio`
- [x] `Config`: `AGENTEVALS_LLM_MODEL` из `.env`
- [x] `.env` / `.env.example`: переменная + комментарий про structured output
- [x] Makefile: `test-deterministic`, `test-llm-judge`, `test-all`

**Проверка:** `make test-all` находит pytest (тесты могут быть пустыми/skip до итераций 65–67).

---

### 65. ДЗ-10: каркас tests/

- [x] `tests/conftest.py` — agent fixture, уникальный `thread_id`
- [x] `tests/helpers.py` — `extract_trajectory`, `print_trajectory`
- [x] Вызов через существующий `AgentService` / `run_turn_agent` (без дублирования агента)
- [x] `AgentService.get_trajectory(chat_id)` для чтения messages из checkpointer

**Проверка:** helper возвращает список messages LangChain после одного хода.

---

### 66. ДЗ-10: детерминированные тесты

- [x] `tests/test_agent_deterministic.py`
- [x] Тест 1: «Что такое гипогликемия?» → `rag_search` (superset, ignore args)
- [x] Тест 2: «Переведи 180 мг/дл в ммоль/л» → `glucose_unit_converter` (superset, ignore args)
- [x] `make test-deterministic` — оба зелёные (+ smoke extract_trajectory)

**Проверка:** match score == 1 на обоих сценариях (локальные tools, без MCP).

---

### 67. ДЗ-10: LLM-as-a-Judge тест

- [x] `tests/test_agent_llm_judge.py`
- [x] Тест 3: «Какие полоски к Contour Plus?» → уместная траектория с `search_products`
- [x] `create_async_trajectory_llm_as_judge` + `AGENTEVALS_LLM_MODEL`
- [x] MCP поднят (`make run-mcp-nika`) или fixture; skip если MCP нет
- [x] `make test-llm-judge` — зелёный

**Проверка:** judge score проходит порог; комментарий evaluator осмысленный.

---

### 68. ДЗ-10: README и полировка

- [x] README: e2e-тесты, команды make, `AGENTEVALS_LLM_MODEL`
- [x] Убедиться что RAGAS-раздел не перепутан с agentevals
- [x] Обновить `report.md` под ДЗ-10 (результаты тестов)
- [x] `.env.example` с `AGENTEVALS_LLM_MODEL`

**Проверка:** `make test-all` проходит; README достаточен для воспроизведения.
