...
from nika.services.embeddings_factory import create_ragas_embeddings
...
def init_ragas_metrics(config: Config) -> tuple[list[Any], Any]:
    ...
    langchain_embeddings = create_ragas_embeddings(config)
    logger.info(
        "RAGAS embeddings: provider=%s model=%s",
...
def evaluation_error_message(error: Exception) -> str:
    from openai import APIStatusError, RateLimitError

    if isinstance(error, RateLimitError):
        return (
            "Превышен лимит запросов к API (rate limit).\n"
            "Подожди несколько минут и повтори, "
            "или смени RAGAS_LLM_MODEL в .env."
        )
    if isinstance(error, APIStatusError) and error.status_code == 429:
        return (
            "Слишком много запросов (429).\n"
            "Подожди и повтори позже."
        )
    if isinstance(error, ValueError):
        return f"Ошибка: {error}"
    return "Не удалось выполнить evaluation. Проверь логи."


class EvaluationService:
    def __init__(self, config: Config, agent: AgentService) -> None:
        self._config = config
        self._agent = agent
    ...
    async def evaluate_dataset(
        ...
        if not self._config.openrouter_api_key:
            msg = (
                ...
                "RAGAS evaluation требует API-ключ для LLM и embeddings."
            )
            raise ValueError(msg)
        ...
        async def target(inputs: dict[str, object]) -> dict[str, object]:
            ...
            result = await self._agent.answer(chat_id, question)
            return {
        ...
        experiment_results = await client.aevaluate(
            ...
            data=resolved_name,
            evaluators=[],
            experiment_prefix="agent-rag-evaluation",
            metadata={
                "approach": "ReAct agent + RAGAS batch evaluation + LangSmith feedback",
                "model": self._config.model_rag,
                "embedding_model": self._config.model_embedding,
                "rag_retrieval_mode": self._config.rag_retrieval_mode,
            },
        )
